...
| Parameters | b0 = 0.0000 (bound), b1 = 0.0363, b2 = 0.0447, b3 = 0.1622, T1 = 1.76, T2 = 15.93 |
...
python/     zcb/                               package: quotes.py, bonds.py, curves.py, analysis.py, report.py
            ...
            build_excel.py                     builds the Excel implementation (live formulas, Solver-ready)
...
4. **Estimate.** Minimise the sum of squared first-order yield errors, each price error divided by dirty
   price times modified duration, with bonds under 3 months to maturity excluded (GSW 2007); 19 multi-start
   least squares runs in Python, and the same objective cell minimised with Solver in Excel.
...
`excel/Miniproject2_ZCB_Term_Structure.xlsx` is laid out the way the model would be built by hand and re-derives
everything from the raw quotes with formulas:

| Sheet | Contents |
|---|---|
| `Data` | WSJ quotes exactly as downloaded |
| `Bonds` | per bond: 32nds conversion, `COUPPCD`/`COUPNCD` coupon dates, accrued interest, dirty price, `YIELD`, `MDURATION`, model price off the curve, price and yield errors, duration-weighted squared error |
| `CashFlows` | every remaining coupon/principal date of every bond (`EDATE`/`EOMONTH`) and its present value `CF x exp(-r(t) t)` |
| `Curve` | the six Svensson parameters (named `beta_0` .. `tau_2`), the Solver objective and fit statistics, the zero-rate / discount-factor / forward table for all 245 payment dates, and the chart |
| `Notes` | the one-page methodology |

Solver instructions sit next to the objective on `Curve`. The workbook is generated by `python/build_excel.py`
so it stays consistent with the data, and `verify_excel.py` confirms (after a recalculation in Excel) that every
Excel model price, yield, duration, zero rate and the objective match the Python results to about 1e-8.
...
Code is MIT licensed. The WSJ / Tullett Prebon quote sheet in `data/` (and its copy under `docs/files/`) is
course material distributed to the class and is included only so the results can be reproduced.
