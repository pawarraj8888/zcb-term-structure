...
def build_paragraphs(results: dict) -> list[tuple[str, str]]:
    ...
    best, worst_32 = conv[0], [c for c in conv if c["price_format"] != "plain decimal"][-1]
    ...
    data = (
        ...
        f"RMSE {best['rmse_bp']:.1f} bp, versus {worst_32['rmse_bp']:.1f} bp for the next-best settlement rule and over "
    ...
    estimation = (
        "The six parameters minimise sum_i [(P_i(model) - P_i(market)) / D_i]^2, squared price errors divided by "
        "modified duration, which to first order equals the sum of squared yield errors (the objective on slide 31) "
        "while keeping each bond's model price a linear sumproduct. Bonds with under {mn:.0f} months to maturity "
        ...
        "{nopt} of them reach the same optimum. In Excel the identical objective is the SSE cell on the Inputs sheet, "
        "minimised with Solver (GRG Nonlinear); the workbook ships at the Python optimum and reproduces every model price."
    ...
    results_text = (
        ...
        f"{rmse_shift:.1f} bp RMSE and moves the in-sample curve by at most {max_shift_bp:.0f} bp, so the discount factors "
        f"on payment dates are robust to it."
    )
    return [
...
def main(argv=None) -> int:
    ...
    (out / "Miniproject2_ZCB_Term_Structure_Writeup.html").write_text(to_html(paragraphs))
